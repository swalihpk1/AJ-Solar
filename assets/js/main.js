...
(function ($) {
  ...
  // Solar Services Interactive Functionality
  $(document).ready(function() {
    ...
    $('.nav-link[href^="#"], .mobile-menu-main .nav-link[href^="#"]').on('click', function(e) {
      e.preventDefault();
      ...
      if (targetSection.length) {
        var offsetTop = targetSection.offset().top - 80; // Account for fixed header
    ...
    // Add active navigation highlighting based on scroll position (both desktop and mobile)
    $(window).on('scroll', function() {
      var scrollTop = $(window).scrollTop();
      var windowHeight = $(window).height();
      
      $('.nav-link[href^="#"], .mobile-menu-main .nav-link[href^="#"]').each(function() {
        var target = $(this).attr('href');
        var targetSection = $(target);
        ...
        if (targetSection.length) {
          ...
          if (scrollTop >= sectionTop && scrollTop < sectionBottom) {
            $('.nav-link, .mobile-menu-main .nav-link').removeClass('active-nav');
            $('.nav-link[href="' + target + '"], .mobile-menu-main .nav-link[href="' + target + '"]').addClass('active-nav');
          }
        }
      });
    });
